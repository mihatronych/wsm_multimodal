        ...
        if glob(f'{path}/segments/*.mp4'):
            ...
            for n, v in enumerate(vs):
                try:
                    ...
                    hc_features_mean_df = pd.DataFrame(columns=[f"hc_{str(e)}_mean" for e in range(len(hc_features_mean_v_mean))], data=[hc_features_mean_v_mean])
                    hc_features_std_df = pd.DataFrame(columns=[f"hc_{str(e)}_std" for e in range(len(hc_features_mean_v_std))], data=[hc_features_mean_v_std])

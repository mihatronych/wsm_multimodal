        ...
        if glob(f'{path}/segments/*.wav'):
            ...
            for n, a in enumerate(auds):
                try:
                    ...
                    hc_features_mean_df = pd.DataFrame(columns=[f"hc_aud_{smile.feature_names[e]}_mean" for e in range(len(smile.feature_names))], data=[hc_features_mean_v_mean])
                    hc_features_std_df = pd.DataFrame(columns=[f"hc_aud_{smile.feature_names[e]}_std" for e in range(len(smile.feature_names))], data=[hc_features_mean_v_std])
